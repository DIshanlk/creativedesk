                ...
                {searchResults.tasks?.length > 0 && (
                  <div className="p-2">
                    ...
                    {searchResults.tasks.map((t: any) => (
                      <button key={t.id} onClick={() => { router.push(`/task/${t.id}`); setShowSearch(false); }} className="w-full text-left px-2 py-1.5 hover:bg-background rounded text-sm flex justify-between">
                ...
                {searchResults.people?.length > 0 && (
                  <div className="p-2 border-t border-border">
                    ...
                    {searchResults.people.map((p: any) => (
                      <button key={p.id} onClick={() => { router.push(`/people`); setShowSearch(false); }} className="w-full text-left px-2 py-1.5 hover:bg-background rounded text-sm">
                ...
                {searchResults.teams?.length > 0 && (
                  <div className="p-2 border-t border-border">
                    ...
                    {searchResults.teams.map((t: any) => (
                      <button key={t.id} onClick={() => { router.push(`/teams`); setShowSearch(false); }} className="w-full text-left px-2 py-1.5 hover:bg-background rounded text-sm">
                        {t.name}
                      </button>
                    ))}
                  </div>
