      ...
      {(overdueTasks.length > 0 || pendingApprovals.length > 0 || overloadedDesigners.length > 0) && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {overdueTasks.length > 0 && (
            <Link href="/list" className="flex gap-3 p-4 bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-900/30 rounded-lg hover:shadow-sm transition-shadow">
          ...
          {pendingApprovals.length > 0 && (
            <Link href="/approvals" className="flex gap-3 p-4 bg-orange-50 dark:bg-orange-900/10 border border-orange-200 dark:border-orange-900/30 rounded-lg hover:shadow-sm transition-shadow">
          ...
          {overloadedDesigners.length > 0 && (
            <Link href="/people" className="flex gap-3 p-4 bg-yellow-50 dark:bg-yellow-900/10 border border-yellow-200 dark:border-yellow-900/30 rounded-lg hover:shadow-sm transition-shadow">
          ...
          <div className="divide-y divide-border">
            {pendingApprovals.slice(0, 5).map((a: any) => (
              <div key={a.id} className="px-5 py-3 flex items-center gap-4 hover:bg-background transition-colors">
                ...
                <Link href="/approvals" className="px-3 py-1.5 bg-primary text-white rounded text-xs font-medium hover:bg-primary/90 shrink-0">Review</Link>
              </div>
            ))}
          </div>
        </div>
              ...
              {overdueTasks.map((task: any) => {
                ...
                return (
                  <tr key={task.id} className="hover:bg-background">
                    <td className="px-5 py-3"><Link href={`/task/${task.id}`} className="text-primary hover:underline font-mono text-xs">{task.key}</Link></td>
    ...
    <div className="space-y-6">
          ...
          {upcoming.length === 0 ? (
            ...
            <div className="space-y-3">
              {upcoming.map(t => {
                ...
                return (
                  <Link key={t.id} href={`/task/${t.id}`} className="flex items-center gap-3 hover:bg-background p-2 -mx-2 rounded-lg transition-colors">
      ...
      {inReview.length > 0 && (
        <div className="bg-surface border border-border rounded-lg overflow-hidden">
          ...
          <div className="divide-y divide-border">
            {inReview.map(t => (
              <Link key={t.id} href={`/task/${t.id}`} className="flex items-center gap-4 px-5 py-3 hover:bg-background transition-colors">
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium truncate">{t.title}</div>
