            ...
            {sortedTasks.map(task => {
              ...
              return (
                <tr key={task.id} className={clsx('hover:bg-background transition-colors', selected.has(task.id) && 'bg-primary/5')}>
                  ...
                  <td className="p-3">
                    <Link href={`/task/${task.id}`} className="text-primary hover:underline font-mono text-xs">{task.key}</Link>
                  </td>
                  <td className="p-3 font-medium max-w-xs">
                    <Link href={`/task/${task.id}`} className="hover:text-primary hover:underline truncate block">{task.title}</Link>
                  </td>
                  <td className="p-3">
                    {canEdit ? (
